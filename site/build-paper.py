...
<!-- Matomo: self-hosted, first-party cookie, IP-anonymized server-side; skipped entirely
     when the browser sends Global Privacy Control (DNT is honored server-side by Matomo).
     Disclosed in privacy.html. NOTE: braces doubled because TEMPLATE is a format string. -->
<script>
if (!navigator.globalPrivacyControl) {{
  var _paq = window._paq = window._paq || [];
  _paq.push(['setTrackerUrl', 'https://analytics.analogelk.com/matomo.php']);
  _paq.push(['setSiteId', '10']);
  _paq.push(['trackPageView']);
  _paq.push(['enableLinkTracking']);
  var g = document.createElement('script');
  g.async = true;
  g.src = 'https://analytics.analogelk.com/matomo.js';
  document.head.appendChild(g);
}}
</script>

<!-- Interaction heatmap: first-party beacon to our own sink (analogelk.com), the same
     infrastructure as the analytics above. Anonymous click positions and scroll depth
     only: no cookies, no PII, no element text. Runs only on the production hostname and
     is skipped entirely when the browser sends Global Privacy Control.
     Disclosed in privacy.html. NOTE: braces doubled because TEMPLATE is a format string. -->
<script>
if (!navigator.globalPrivacyControl) {{
  (function () {{
    ...
    if (location.hostname !== 'musterr.dev' && location.hostname !== 'www.musterr.dev') return;

    var ENDPOINT = 'https://analogelk.com/api/interactions';
    var MAX_CLICK_BEACONS = 200; // storm cap per page load
    var SITE = 'musterr.dev';

    var device = /Mobi|Android|iPhone|iPad|iPod|Tablet/i.test(navigator.userAgent)
      ? 'mobile'
      : 'desktop';
    ...
    // Clicks: one capture-phase listener, viewport-relative X, document-relative Y.
    var clickCount = 0;
    document.addEventListener('click', function (e) {{
      if (clickCount >= MAX_CLICK_BEACONS) return;
      clickCount += 1;
      var scrollHeight = document.documentElement.scrollHeight || document.body.scrollHeight;
      var xPct = (e.clientX / window.innerWidth) * 100;
      var yPct = scrollHeight > 0 ? ((e.clientY + window.scrollY) / scrollHeight) * 100 : 0;
      beacon({{
        event_type: 'click',
    ...
    // Scroll depth: rAF-throttled max tracker, one beacon per page view,
    // flushed on pagehide or tab-hide (whichever comes first).
    var maxScroll = 0;
    var ticking = false;
    ...
    function currentScrollPct() {{
      ...
      var raw = ((window.scrollY + window.innerHeight) / scrollHeight) * 100;
