...
The apparatus is public, MIT-licensed (© Michael Walliser; the `os_*` schema's lineage from directus-labs/agency-os is attributed in `NOTICE`), and self-hostable — permissive on purpose: the business is services, and the code is the credential, not the moat. The one-command flow is the whole pitch, and the `doctor` board at the end is the credibility instrument:
